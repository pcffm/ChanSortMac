...
namespace ChanSort.Loader.Toshiba
{
  ...
  {
    private static string ChmgtDb(string root) => Path.Combine(root, "chmgt_type001", "chmgt.db");
    private static string DvbSysDataDb(string root) => Path.Combine(root, "dvb_type001", "dvbSysData.db");
    private static string DvbMainDataDb(string root) => Path.Combine(root, "dvb_type001", "dvbMainData.db");
    ...
    public override void Load()
    {
      ...
      var sysDataConnString = $"Data Source={DvbSysDataDb(this.workingDir)};Pooling=false";
      ...
      var mainDataConnString = $"Data Source={DvbMainDataDb(this.workingDir)};Pooling=False";
      ...
      var channelConnString = $"Data Source={ChmgtDb(this.workingDir)};Pooling=False";
